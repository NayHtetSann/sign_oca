odoo.define('sign_oca.views_custo', function(require) {
    'use strict';
    ...
    KanbanRecord.include({
        ...
        _openRecord: function () {
            var self = this;
            if (this.modelName === 'sign.oca.template' && this.$el.parents('.o_sign_template_kanban').length) {
            ...
            } else if (this.modelName === 'sign.oca.request' && this.$el.parents('.o_sign_request_kanban').length) {
                this._rpc({
        ...
        async _render() {
            ...
            if (config.device.isMobile &&
                (this.modelName === "sign.oca.template" || this.modelName === "sign.oca.request")) {
    ...
    function _make_custo(selector_button) {
        return {
            renderButtons: function () {
                ...
                if (this.modelName === "sign.oca.template") {
                    this._sign_upload_file_button();
                    this.$buttons.find('button.o_button_import').hide();

                } else if (this.modelName === "sign.oca.request") {
                    if (this.$buttons) {
...
odoo.define('sign.oca.template', function(require) {
    'use strict';
    ...
    var PDFIframe = require('sign_oca.PDFIframe');
    var sign_utils = require('sign_oca.utils');
    ...
    var SignItemCustomPopover = Widget.extend({
        template: 'sign_oca.sign_item_custom_popover',
        events: {
    ...
    var InitialAllPagesDialog = Dialog.extend({
        template: 'sign_oca.initial_all_pages_dialog',
            ...
            this.fullyLoaded.then(function() {
                if(self.editMode) {
                    if(self.$iframe.prop('disabled')) {
                        ...
                        var $fieldTypeButtons = $(core.qweb.render('sign_oca.type_buttons', {sign_item_types: typesArr}));
        ...
        enableCustom: function($signatureItem) {
            ...
            $configArea.find('.o_sign_item_display').off('mousedown').on('mousedown', function(e) {
                ...
                if (self.customPopovers[itemId]) {
                    ...
                    self.customPopovers[itemId] = new SignItemCustomPopover(self, self.parties, {'field_name': $signatureItem[0]['field-name'], 'item_type': $signatureItem[0]['field-type']}, self.select_options);
    ...
    var Template = AbstractAction.extend(StandaloneFieldManagerMixin, {
        ...
        events: {
            ...
            'click .o_sign_template_send' : function (e) {
                this.do_action('sign_oca.action_sign_oca_send_request', {
                    additional_context: {
            ...
            'click .o_sign_template_share' : function (e) {
                this.do_action('sign_oca.action_sign_template_share', {
                    additional_context: {
                        active_id: this.templateID,
                    },
                });
        ...
        renderButtons: function () {
            this.$buttons = $(core.qweb.render("sign_oca.template_cp_buttons", {'widget':this, 'action_type':this.actionType}));
        },
        ...
        perform_rpc: function() {
            ...
            var defTemplates = this._rpc({
                ...
                .then(function prepare_template(template) {
                    ...
                    self.has_sign_requests = (template.request_ids.length > 0);
        ...
        initialize_content: function() {
            ...
            this.debug = config.isDebug();
            this.$('.o_content').append(core.qweb.render('sign_oca.template', {widget: this}));
        ...
        prepareTemplateData: function() {
            ...
            for(var page in configuration) {
                for(var i = 0 ; i < configuration[page].length ; i++) {
                    ...
                    data[id] = {
                        'field_id': configuration[page][i].data('type'),
    ...
    core.action_registry.add('sign.oca.Template', Template);
});

odoo.define('sign_oca.DocumentBackend', function (require) {
    'use strict';
    var AbstractAction = require('web.AbstractAction');
    var core = require('web.core');
    var Document = require('sign_oca.Document');
    var framework = require('web.framework');
    ...
    var DocumentBackend = AbstractAction.extend({
        ...
        go_back_to_kanban: function () {
            return this.do_action("sign_oca.sign_oca_request_act_window", {
                clear_breadcrumbs: true,
            });
        },
        ...
        start: function () {
            ...
            var def = this._rpc({
                route: '/sign_oca/get_document/' + this.documentID + '/' + this.token,
...
odoo.define('sign_oca.document_edition', function(require) {
    'use strict';

    var core = require('web.core');
    var session = require('web.session');
    var DocumentBackend = require('sign_oca.DocumentBackend');
    ...
    core.action_registry.add('sign_oca.Document', EditableDocumentBackend);
});

odoo.define('sign_oca.document_signing_backend', function(require) {
    'use strict';
    ...
    var NoPubThankYouDialog = document_signing.ThankYouDialog.extend({
        template: "sign_oca.no_pub_thank_you_dialog",
    ...
    core.action_registry.add('sign_oca.SignableDocument', SignableDocumentBackend);
});
