odoo.define('sign_oca.PDFIframe', function (require) {
    'use strict';
    ...
    var PDFIframe = Widget.extend(Object.assign({}, PinchItemMixin, {
        ...
        doPDFPostLoad: function() {
            ...
            var assets_def = this._rpc({
                route: '/sign_oca/render_assets_pdf_iframe',
                params: {
                    args: [{
            ...
            $(Object.keys(this.signatureItems).map(function(id) { return self.signatureItems[id]; }))
                .sort(function(a, b) {
                    ...
                    var $signatureItem = self.createSignItem(
                        self.types[parseInt(el.type || el.field_id[0])],
                        !!el.required,
                        parseInt(el.responsible || el.role_id[0]) || 0,
                        parseFloat(el.posX),
                        parseFloat(el.posY),
                        parseFloat(el.width),
        ...
        createSignItem: function (type, required, responsible, posX, posY, width, height, value, option_ids, name, tooltip) {
            ...
            var $signatureItem = $(core.qweb.render('sign_oca.sign_item', {
...
odoo.define('sign_oca.Document', function (require) {
    'use strict';
...
odoo.define('sign_oca.utils', function (require) {
    'use strict';
...
// Signing part
odoo.define('sign_oca.document_signing', function (require) {
    'use strict';
    ...
    var SignNameAndSignature = NameAndSignature.extend({
        ...
        willStart: function () {
            ...
            return Promise.all([
                this._super.apply(this, arguments),
                self._rpc({
                    route: '/sign_oca/get_signature/' + self.requestID + '/' + self.accessToken,
                    params: {
    ...
    var SignatureDialog = Dialog.extend({
        template: 'sign_oca.signature_dialog',
    ...
    var PublicSignerDialog = Dialog.extend({
        template: "sign_oca.public_signer_dialog",
        ...
        init: function(parent, requestID, requestToken, RedirectURL, options) {
            ...
            if(!options.buttons) {
                ...
                options.buttons.push({text: _t("Validate & Send"), classes: "btn-primary", click: function(e) {
                    ...
                    ajax.jsonRpc("/sign_oca/send_public/" + this.requestID + '/' + this.requestToken, 'call', {
                        name: name,
                        mail: mail,
                    }).then(function() {
                        self.close();
    ...
    var SMSSignerDialog = Dialog.extend({
        template: "sign_oca.public_sms_signer",

        events: {
            'click button.o_sign_resend_sms': function(e) {
                var $btn = self.$('.o_sign_resend_sms');
                $btn.attr('disabled', true);
                var route = '/sign_oca/send-sms/' + this.requestID + '/' + this.requestToken + '/' + this.$('#o_sign_phone_number_input').val();
        ...
        _onValidateSMS: function () {
            ...
            var route = '/sign_oca/sign/' + this.requestID + '/' + this.requestToken + '/' + input.val();
            var params = {
    ...
    var EncryptedDialog = Dialog.extend({
        template: "sign_oca.public_password",
        ...
        _onValidatePassword: function () {
            ...
            var route = '/sign_oca/password/' + this.requestID ;
            var params = {
                password: input.val()
            };
            var self = this;
    ...
    var ThankYouDialog = Dialog.extend({
        ...
        init: function(parent, RedirectURL, RedirectURLText, requestID, options) {
            ...
            this._rpc({
                route: '/sign_oca/encrypted/' + requestID
        ...
        start: async function () {
            ...
            if (result && result.length) {
                ...
                this.next_document = result.reduce(function (prev, curr) {
                    return (Math.abs(curr.request_id[0] - self.requestID) <= Math.abs(prev.request_id[0] - self.requestID) ? curr : prev);
                });
                this.options.buttons.push({
                    text: _t('Sign Next Document'), classes: 'btn-primary', click: function (e) {
                        self._rpc({
                            model: 'sign.oca.request',
                            method: 'go_to_document',
                            args: [self.next_document.request_id[0]],
                        }).then(function(action) {
                            self.do_action(action, {clear_breadcrumbs: true});
                        });
                    }
            ...
            this.setElement($(core.qweb.render('sign_oca.thank_you_dialog', {widget: this})));
    ...
    var NextDirectSignDialog = Dialog.extend({
        template: "sign_oca.next_direct_sign_dialog",
        events: {
            'click .o_go_to_document': 'on_closed',
    ...
    var SignableDocument = Document.extend({
        ...
        signItemDocument: function(e) {
            ...
            function _sign() {
                ...
                var route = '/sign_oca/sign/' + this.requestID + '/' + this.accessToken;
                var params = {
        ...
        signDocument: function (e) {
            ...
            signDialog.open().onConfirm(function () {
                ...
                function _sign() {
                    ajax.jsonRpc('/sign_oca/sign/' + self.requestID + '/' + self.accessToken, 'call', {
                        signature: signature,
                ...
                return documentPage.attachTo($('body')).then(function() {
                    ...
                    if(askLocation && navigator.geolocation) {
                        navigator.geolocation.getCurrentPosition(function(position) {
                            ...
                            ajax.jsonRpc('/sign_oca/save_location/' + documentPage.requestID + '/' + documentPage.accessToken, 'call', coords);
                        });
                    }
                });
            });
